...
public class Chunk extends ChunkEntities implements IChunk {
    ...
    private static String hexPreview(DataTypeProvider p, int max) {
        int pos = p.getPosition();
        int n = Math.min(max, p.remaining());
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < n; i++) {
            sb.append(String.format("%02x ", p.peekAt(pos + i)));
        }
    ...
    protected void parse(DataTypeProvider dataProvider) {
        ...
        String packetHeadHex = hexPreview(dataProvider, 48);

        parseHeightMaps(dataProvider);

        int size = dataProvider.readVarInt();
        String columnHeadHex = hexPreview(dataProvider, 48);

        try {
            DataTypeProvider columnReader = dataProvider.ofLength(size);
            readChunkColumn(columnReader);
            if (columnReader.hasNext()) {
                System.err.println("[chunk-debug] " + location + ": " + columnReader.remaining()
                    + " unread byte(s) left in chunk column — section parsing stopped early");
            }
        ...
        int sectionCount = 0;
        int nonAirBlocks = 0;
        ChunkSection firstSection = null;
        for (ChunkSection section : getAllSections()) {
            if (firstSection == null) { firstSection = section; }
            sectionCount++;
            nonAirBlocks += Math.max(0, section.getBlockCount());
        }
        if (sectionCount == 0 || nonAirBlocks == 0) {
            String paletteInfo = "";
            if (firstSection != null && firstSection.getBlockPalette() != null) {
                Palette pal = firstSection.getBlockPalette();
                long[] bdata = firstSection.getBlocks();
                int nonZeroLongs = 0;
                if (bdata != null) {
                    for (long l : bdata) { if (l != 0) { nonZeroLongs++; } }
                }
                paletteInfo = ", first section palette: " + pal.size() + " entr(ies) at "
                    + pal.getBitsPerBlock() + " bits, data longs="
                    + (bdata == null ? "null" : bdata.length + " (" + nonZeroLongs + " non-zero)");
            }
            System.err.println(
                "[chunk-debug] chunk " + location + " parsed with " + sectionCount + " section(s) and "
                    + nonAirBlocks + " non-air block(s) (declared column size=" + size + " bytes"
                    + paletteInfo + ") — it will save as empty terrain"
            );
            System.err.println("[chunk-dump] " + location + " after x,z: " + packetHeadHex);
            System.err.println("[chunk-dump] " + location + " column head: " + columnHeadHex);
        }
    ...
    public void readChunkColumn(DataTypeProvider dataProvider) {
        for (int sectionY = getMinBlockSection(); sectionY <= getMaxBlockSection(); sectionY++) {
            if (!dataProvider.hasNext()) {
                System.err.println("[chunk-debug] " + location + ": chunk data buffer exhausted at section "
                    + sectionY + " of " + getMinBlockSection() + ".." + getMaxBlockSection()
                    + " — remaining sections will be empty (size field or earlier fields misread)");
                break;
            }
    ...
    protected boolean containsBlockEntities(Palette p) {
        ...
        for (SpecificTag tag : p.toNbt()) {
            if (blockEntities.isBlockEntity(tag.get("id").stringValue())) {
                return true;
            }
        }
        return false;
